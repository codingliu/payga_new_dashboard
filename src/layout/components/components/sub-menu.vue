...
<template>
  <a-sub-menu :key="menuInfo.name">
    <template #title>
      {{ appStore.i18n ? (menuInfo.meta.title ? menuInfo.meta.title : $t(`menus.${menuInfo.name}`)) : menuInfo.meta.title }}
    </template>
    ...
    <template v-for="item in menuInfo.children" :key="item.id">
      <template v-if="!item.children|| item.children.length === 0">
        <a-menu-item :key="item.name" @click="routerPush(item)">
          ...
          {{ appStore.i18n ? (item.meta.title ? item.meta.title : $t(`menus.${item.name}`)) : item.meta.title }}
        </a-menu-item>
      </template>
      <template v-else>
...
<script setup name="SubMenu">
  ...
  const props = defineProps({ menuInfo: Object })

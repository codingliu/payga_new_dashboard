    ...
    <template v-if="isFunction(row.hide) ? row.hide() : !row.hide">
      ...
      >
        <template #cell="{ record, column, rowIndex }">
          <!-- 操作栏 -->
          <template v-if="row.dataIndex === '__operation' && !row.hide">
          ...
          <template v-else-if="row.customRender && !row.hide">
            <custom-render
                :column="column"
                :record="record"
          ...
          <slot :name="row.dataIndex" v-bind="{ record, column, rowIndex }" v-else-if="!row.hide">

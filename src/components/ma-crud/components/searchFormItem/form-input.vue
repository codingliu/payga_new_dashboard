...
<template>
  <a-input-group v-if="props.component.formType === 'input-number' && (props.component?.rangeSearch ?? false)">
    <a-input-number
      v-model="minData"
      allow-clear
      :placeholder="props.component.searchPlaceholder ?? `${tr('请输入')}${props.component.title}${tr('最小值')}`"
    />
    <span class="p-1">~</span>
    <a-input-number
      v-model="maxData"
      allow-clear
      :placeholder="props.component.searchPlaceholder ?? `${tr('请输入')}${props.component.title}${tr('最大值')}`"
    />
  </a-input-group>
  ...
  <a-input-number
    ...
    v-model="value"
    :placeholder="props.component.searchPlaceholder ?? `${tr('请输入')}${props.component.title}`"
    allow-clear
  />
  <a-input
    v-else

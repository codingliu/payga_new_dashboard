...
<template>
  <a-select
    ...
    :placeholder="props.component.searchPlaceholder ?? `${tr('请选择')}${props.component.title}`"
    allow-clear
    allow-search
  ...
  >
    <template #header v-if="props.component.multiple">
      <div style="padding: 6px 12px;" >
        <a-space>
          <a-checkbox :value="false" @change="handleSelectAll">{{ tr('全选/清除') }}</a-checkbox>
          <a-button size="mini" type="outline" @click="handleInverse">{{ tr("反选") }}</a-button>
        </a-space>
      </div>
    </template>
      ...
      <div class="flex justify-center">
        ...
        >
          <template #page-item-step="{ type }">
            <div>{{ type === 'previous' ? tr('上一页') : tr('下一页') }}</div>
          </template>
        </a-pagination>
      </div>
    </template>

...
var odata = isDevelop ?
    { username: 'test@test.com', password: '123456', authCode: '' }
...
const redirect = route.query.redirect ? route.query.redirect : tool.local.get('homePath') || '/'
const appStore  = useAppStore()
const formlocale = reactive({
  ...appStore,
})
...
const handleSubmit = async ({ values, errors }) => {
  ...
  if (!errors) {
    ...
    params.password = md5.end()
    params.merchantNo = params.username
  ...
  <div class="login-container">
    <div class="login-width md:w-10/12 w-11/12 mx-auto flex justify-between h-full items-center">
      <div class="w-6/12 mx-auto left-panel rounded-l pl-5 pr-5 hidden md:block">
        <div class="logo"><img :src="`${$url}logo.png`" width="45"><span>{{ $title }}</span></div>
